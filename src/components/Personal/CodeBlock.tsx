...
const CodeBlock = ({ code, language = "tsx" }: CodeBlockProps) => {
  ...
  return (
    <div className="relative">
      ...
      <pre className="border border-t-0 border-gray-200 dark:border-gray-800 p-4 rounded-b-md overflow-x-auto" style={{ backgroundColor: "var(--card-bg, #f9fafb)" }}>
        <code className="text-sm font-mono opacity-90" style={{ color: "var(--text-color)" }}>{code}</code>
      </pre>
    </div>
  );
};
